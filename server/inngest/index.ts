...
const sendMonthlyOffers = inngest.createFunction({
    ...
    if (users.length === 0 || deals.length === 0) {
        return { skipped: true, reason: "Sin usuarios ni ofertas" };
    }
...
const autoAssignRider = inngest.createFunction({
    ...
    const result = await step.run("assign-rider", async () => {
        ...
        if(!order) return {skipped: true, reason: "Orden no Encontrada"};
        if(order.deliveryPartnerId) return {skipped: true, reason: "Ya asignado"};
        if(["Cancelado", "Entregado"].includes(order.status as string)) return {skipped: true, reason: `Orden esta ${order.status}`};

        //Find an active rider not currently delivering
        const busyOrders = await prisma.order.findMany({
            where: {
                status: {in: ["Asignado", "Empaquetado", "En reparto"]},
        ...
        if(!availableRider) return {skipped: true, reason: "No hay conductores disponibles"}
