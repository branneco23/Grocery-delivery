...
export const createOrder = async (req: Request, res: Response) => {
    ...
    if (!items || items.length === 0) {
        return res.status(400).json({ message: "Sin artículos en el pedido" })
    }
    ...
    for (const item of items) {
        ...
        if (!product || (product.stock ?? 0) < item.quantity) {
            return res.status(404).json({ message: "Producto agotado" });
        }
    }
    ...
    // IVA general en Colombia: 19% (0.19)
    ...
    const order = await prisma.order.create({
        data: {
            ...
            tax,
            total,
            statusHistory: [{status: "Ubicación", note: "Pedido realizado con éxito", timestamp: new Date()}]
        }
    })
...
export const updateOrderStatus = async(req: Request, res: Response) => {
    ...
    history.push({status, note: note || `Orden ${status.toLowerCase()}`, timeStamp: new Date()})
