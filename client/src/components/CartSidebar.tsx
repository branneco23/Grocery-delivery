import { ArrowRightIcon, MinusIcon, PlusIcon, ShoppingBag, Trash2Icon, XIcon } from "lucide-react";
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-md bg-white shadow-2xl z-50 flex flex-col animate-slide-in-right">
        ...
        <div className="flex items-center justify-between p-5 border-b border-app-border">
          <div className="flex items-center gap-2">
            ...
            <span className="px-2 py-0.5 text-xs font-semibold bg-app-cream rounded-full">{items.length} Productos</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              <ShoppingBag className="size-16 text-app-border mb-4" />
              <h3 className="text-lg font-medium mb-1">Tu carrito está vacío.</h3>
            </div>
          ) : (
            items.map((item) => (
              <div key={item.product._id} className="flex gap-3 bg-app-border/60 rounded-xl p-3">
                <img src={item.product.image} alt={item.product.name} className="size-16 rounded-lg object-cover shink-0"/>
        ...
        {items.length > 0 && (
          <div className="p-5 border-t border-app-border space-y-3">
            ...
            <div className="flex justify-between text-sm">
              <span className="text-app-text-light">Domicilios</span>
              <span className="font-medium">{deliveryFee === 0 ? <span className="text-app-success">Gratis</span> : `${currency}${deliveryFee.toFixed(2)}`}</span>
            </div>

            {deliveryFee > 0 && <p className="text-xs text-app-text-light text-center">Domicilio Grátis sobre ordenes superiores a {currency}20!</p>}
            ...
            <button onClick={()=>{setIsCartOpen(false); navigate('/checkout'); window.scrollTo(0,0)}} className="w-full py-3 bg-app-orange text-white font-semibold rounded-xl hover:bg-app-orange-dark transition-colors flex-center gap-2 active:scale-[0.98]">
              Pasar por la caja <ArrowRightIcon className="size-4"/>
            </button>
          </div>
        )}
      </div>
