...
import { ChevronDown, Home as HomeIcon, SlidersHorizontal, XIcon } from 'lucide-react'; // 2. Renombrado para que no choque con la página Home
        ...
        <div className="flex gap-8 xl:gap-10">
          ...
          <main className='flex-1'>
            ...
            <div className="flex items-center justify-between mb-6">
              <div>
                <h1 className='text-2xl font-semibold text-app-green'>{activeCategory ? activeCategory.name : "Todos los Productos"}</h1>
                <p className='text-sm text-app-text-light mt-0.5'>{products.length} productos encontrados</p>
              </div>

              <div className='flex flex-col lg:items-center gap-3'>
                {/* Mobile filter toggle */}
                <button onClick={() => setMobileFiltersOpen(true)} className='lg:hidden flex items.center gap-2 px-3 py-2 hover:bg-app-cream transition-colors'>
            ...
            {loading ? (
              ...
              <div className='text-center py-16'>
                <p className='text-lg font-semibold text-app-green mb-2'>Producto No Encontrado</p>
                <p className='text-sm text-app-text-light mb-4'>Prueba a ajustar los filtros o los términos de búsqueda.</p>
                <button onClick={clearFilters} className='px-5 py-2 text-sm font-medium bg-app-green text-white rounded-xl hover:bg-app-green-light transition-colors'>Limpiar Filtros</button>
              </div>
            ) : (
